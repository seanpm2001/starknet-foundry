...
use shared::consts::{EXPECTED_RPC_VERSION, RPC_URL_WITH_NO_VERSION};
...
fn should_print_warning() {
    ...
    let mut rpc_url: Url = RPC_URL_WITH_NO_VERSION.clone();
    rpc_url.set_path("/rpc/v0_5");
    ...
    assert_stdout_contains(
        ...
        formatdoc!(
            r"
                [..]Compiling[..]
                [..]Finished[..]
                [WARNING] RPC node with the url {rpc_url} uses incompatible version 0.5.1. Expected version: {EXPECTED_RPC_VERSION}
                ...
                Latest block number = [..] for url = {rpc_url}
...
fn should_print_foreach() {
    ...
    temp.child("tests/test.cairo")
        .write_str(formatdoc!(
            r#"
                #[fork(url: "http://127.0.0.1:3030?url={rpc_url}", block_id: BlockId::Tag(BlockTag::Latest))]
                #[test]
                fn t1() {{
                    assert!(false);
                }}
    ...
    assert_stdout_contains(
        ...
        formatdoc!(
            r"
                [..]Compiling[..]
                [..]Finished[..]
                [WARNING] RPC node with the url http://127.0.0.1:3030?url={rpc_url} uses incompatible version 0.5.1. Expected version: {EXPECTED_RPC_VERSION}
                [WARNING] RPC node with the url {rpc_url} uses incompatible version 0.5.1. Expected version: {EXPECTED_RPC_VERSION}
                ...
                Latest block number = [..] for url = http://127.0.0.1:3030?url={rpc_url}
                Latest block number = [..] for url = {rpc_url}
